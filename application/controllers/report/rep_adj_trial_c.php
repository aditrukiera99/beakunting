...
class Rep_adj_trial_c extends CI_Controller {
	...
	public function index(){
		...
		$dt = $this->db->query("
			...
			WHERE STR_TO_DATE(TGL, '%d-%c-%Y') <= STR_TO_DATE('$tgl_akhir' , '%d-%c-%Y') AND STR_TO_DATE(TGL, '%d-%c-%Y') >= STR_TO_DATE('$tgl_awal' , '%d-%c-%Y') AND ADJ = 1
			ORDER BY ID ASC
		")->result();
		...
		$data = array(
			'page' => 'report/rep_adj_trial_v', 
			'dt' => $dt,
			'sel_date' => $sel_date,
			'tgl_awal' => $tgl_awal,
			'tgl_akhir' => $tgl_akhir,
			'post_url' => 'report/rep_adj_trial_c',
		);
