...
class Credit_vendor_c extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('vend_id')){
			...
			$this->db->query("
				...
				VALUES 
				('$ref_no', '$tgl', '$memo', '$vend_name', 'Credit Vendor', '$id_pembelian')
			");
	...
	public function detail($id){
		...
		$cek_data = count($this->db->query("SELECT * FROM ak_pembelian WHERE ID = '$id' AND TIPE = 'Credit Vendor' ")->result());
		if($cek_data == 0){
			redirect(base_url()."credit_vendor_c");
		}
		...
		$data = array(
			...
			'post_url' => 'credit_vendor_c/detail/'.$id, 
		);
